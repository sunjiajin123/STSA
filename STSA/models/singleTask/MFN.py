...
class MFN(nn.Module):
	def __init__(self, args):
		...
		self.d_l,self.d_a,self.d_v = args.feature_dims           #768  33  709
		self.dh_l,self.dh_a,self.dh_v = args.hidden_dims         #128   16  128
		total_h_dim = self.dh_l+self.dh_a+self.dh_v              #272
		self.mem_dim = args.memsize								 #128
		window_dim = args.windowsize							 # 2    两个时间步
		output_dim = args.num_classes if args.train_mode == "classification" else 1
		attInShape = total_h_dim*window_dim                      #272*2
		gammaInShape = attInShape+self.mem_dim					 #272*2+128
		final_out = total_h_dim+self.mem_dim                     #272+128
		h_att1 = args.NN1Config["shapes"]						 #64
		h_att2 = args.NN2Config["shapes"]						 #64
		h_gamma1 = args.gamma1Config["shapes"]                   #256
		h_gamma2 = args.gamma2Config["shapes"]                   #256
		h_out = args.outConfig["shapes"]						 #64
		...
		self.lstm_l = nn.LSTMCell(self.d_l, self.dh_l)         #单层
	...
	def forward(self, text_x, audio_x, video_x):
		...
		'''
		text_x = text_x.permute(1,0,2)            #(39,64,768)
		audio_x = audio_x.permute(1,0,2)          #(39,64,33)
		video_x = video_x.permute(1,0,2)		  #(39,64,709)

		# x is t x n x d
		n = text_x.size()[1]                      #批度
		t = text_x.size()[0]                      #序列长度
		...
		for i in range(t):
			...
			cStar = torch.cat([prev_cs,new_cs], dim=1)                                       #两个时间步拼接后的c[t-1,t]
			attention = F.softmax(self.att1_fc2(self.att1_dropout(F.relu(self.att1_fc1(cStar)))),dim=1)
			attended = attention*cStar														 #加权后的c‘[t-1,t]
			cHat = torch.tanh(self.att2_fc2(self.att2_dropout(F.relu(self.att2_fc1(attended))))) #(64,128)
			both = torch.cat([attended,self.mem], dim=1)                                         #(64,272*2+128)
