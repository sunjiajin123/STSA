...
class SELF_MM():
    def __init__(self, args):
        ...
        # torch.Size([1368, 128])       Linear隐藏层的特征数
        # torch.Size([1368, 64])
        # torch.Size([1368, 16])
        # torch.Size([1368, 32])
    ...
    def do_train(self, model, dataloader):
        ...
        bert_params_decay = [p for n, p in bert_params if not any(
            nd in n for nd in bert_no_decay)]  # 'bias', 'LayerNorm.bias', 'LayerNorm.weight'不需要权重衰减
        ...
        while True:  ##看到这
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # forward
                    device = torch.device('cpu')  # 我自己加的
                    audio_lengths = audio_lengths.to(device)  # 我自己加的
                    vision_lengths = vision_lengths.to(device)  # 我自己加的
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if not self.args.need_data_aligned:
                        ...
                        audio_lengths, vision_lengths = 0, 0
                    labels={}
                    labels['M'] = batch_data['labels']['M'].to(self.args.device).view(-1)
                    labels['T'] = batch_data['labels']['T'].to(self.args.device).view(-1)
                    labels['A'] = batch_data['labels']['A'].to(self.args.device).view(-1)
                    labels['V'] = batch_data['labels']['V'].to(self.args.device).view(-1)
                    device = torch.device('cpu')  # 我自己加的
                    audio_lengths = audio_lengths.to(device)  # 我自己加的
                    vision_lengths = vision_lengths.to(device)  # 我自己加的
                    outputs = model(text, (audio, audio_lengths), (vision, vision_lengths))
                    loss = self.weighted_loss(outputs['M'], labels['M'])
                    eval_loss += loss.item()
                    for m in self.args.tasks:
                        y_pred[m].append(outputs[m].cpu())
